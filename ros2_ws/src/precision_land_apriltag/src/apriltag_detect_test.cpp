...
class ReturnToHomeControl : public rclcpp::Node
{
public:
    ReturnToHomeControl() : Node("return_to_home_control")
    {
        ...
        vehicle_local_position_sub_ =
            this->create_subscription<VehicleLocalPosition>(
                ...
                std::bind(&ReturnToHomeControl::localPositionCallback, this, std::placeholders::_1));

        vehicle_attitude_sub_ =
            this->create_subscription<VehicleAttitude>(
                "/fmu/out/vehicle_attitude", qos_be,
                std::bind(&ReturnToHomeControl::attitudeCallback, this, std::placeholders::_1));

        land_detected_sub_ =
            this->create_subscription<VehicleLandDetected>(
                "/fmu/out/vehicle_land_detected", qos_be,
                std::bind(&ReturnToHomeControl::landDetectedCallback, this, std::placeholders::_1));

        apriltag_sub_ =
            this->create_subscription<apriltag_msgs::msg::AprilTagDetectionArray>(
                "/detections",
                rclcpp::QoS(10).best_effort(),
                std::bind(&ReturnToHomeControl::apriltagCallback, this, std::placeholders::_1));

        camera_info_sub_ =
            this->create_subscription<sensor_msgs::msg::CameraInfo>(
                "/camera/camera_info",
                rclcpp::QoS(10).best_effort(),
                std::bind(&ReturnToHomeControl::cameraInfoCallback, this, std::placeholders::_1));
        ...
        timer_ = this->create_wall_timer(100ms, std::bind(&ReturnToHomeControl::timerCallback, this));
        RCLCPP_INFO(this->get_logger(), "ReturnToHome started");
    }

private:
    // ── Parameters ────────────────────────────────────────────────────────────
    static constexpr int    TARGET_TAG_ID    = 9;     // tag at home (0,0)
    ...
    enum class Stage {
        INIT,
        TAKEOFF,          // climb to 5 m above (0,0)
        WAIT_FOR_TAG,     // hover at 5 m, wait for tag 9 lock
        DESCEND,          // precision vision-guided descent onto tag 9
        COMMIT_LAND,      // below 0.3 m — NAV_LAND + disarm
        FINISHED
    };
    ...
    std::string stageName(Stage s) const
    {
        switch (s) {
            ...
            case Stage::WAIT_FOR_TAG: return "WAIT_FOR_TAG";
    ...
    void apriltagCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg)
    {
        ...
        if (stage_ == Stage::INIT ||
            ...
            stage_ == Stage::COMMIT_LAND ||
            stage_ == Stage::FINISHED) return;
        ...
        for (const auto & det : msg->detections) {
            ...
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 300,
                "TAG 9  raw=(%.2f,%.2f)  ema=(%.2f,%.2f)  drone=(%.2f,%.2f)",
    ...
    void timerCallback()
    {
        ...
        case Stage::TAKEOFF:
        {
            ...
            if (arrivedAt(TAKEOFF_X, TAKEOFF_Y, TAKEOFF_ALT)) {
                resetTag();
                switchToStage(Stage::WAIT_FOR_TAG);
            }
            break;
        }

        // ── 2. Hover at 5 m, wait for tag 9 lock ─────────────────────────────
        case Stage::WAIT_FOR_TAG:
        {
            publishTrajectorySetpointPos(TAKEOFF_X, TAKEOFF_Y, TAKEOFF_ALT);

            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                "WAIT_FOR_TAG  alt=%.2f  tag_valid=%d",
                alt, (int)tag_.valid());

            if (!target_lost) {
                RCLCPP_INFO(this->get_logger(), "Tag 9 locked — starting precision descent");
                switchToStage(Stage::DESCEND);
            }
            break;
        }

        // ── 3. Precision vision-guided descent onto tag 9 ─────────────────────
        case Stage::DESCEND:
        {
            ...
            if (target_lost) {
                ...
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                    "Tag 9 lost — holding (%.2f,%.2f,%.2f)", pos_.x, pos_.y, pos_.z);
                break;
            }
        ...
        // ── 4. Below 0.3 m — NAV_LAND then disarm ────────────────────────────
        ...
        case Stage::FINISHED:
        {
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                "Mission complete — landed on tag 9 and disarmed.");
            break;
        }
        }
    }
...
int main(int argc, char * argv[])
{
    ...
    rclcpp::spin(std::make_shared<ReturnToHomeControl>());
    rclcpp::shutdown();
    return 0;
}